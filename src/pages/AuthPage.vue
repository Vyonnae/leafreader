...
const view = ref(props.resetPassword ? 'update' : 'auth')
...
const heading = computed(() => ({
  auth: authMode.value === 'sign-up' ? 'Create a home for your reading.' : 'Welcome back to your reading space.',
  forgot: 'Find your way back in.',
  update: 'Choose a new password.',
}[view.value]))

const introduction = computed(() => ({
  auth: authMode.value === 'sign-up'
  ...
  forgot: 'We will send a secure reset link to the email on your account.',
  update: 'Use at least 8 characters, then return to your reading space.',
}[view.value]))
...
function readerDestination() {
  ...
  if (!resolved.name || ['auth', 'reset-password'].includes(String(resolved.name))) return '/app'
  return redirect
}
...
function showAuth() {
  ...
  if (props.resetPassword) router.push('/auth')
...
watch(() => props.resetPassword, (isReset) => {
  view.value = isReset ? 'update' : 'auth'
  clearMessages()
})
</script>
    ...
    <section class="auth-content">
      <div class="auth-content-inner">
        ...
        <AuthForm
          ...
          @forgot-password="clearMessages(); view = 'forgot'"
          @continue-guest="router.push('/app')"
        />
        <ForgotPasswordForm
          v-else
          :mode="view === 'update' ? 'update' : 'request'"
          :configured="isSupabaseConfigured"
          :busy="busy"
          :error="error"
          :success="success"
          @submit="view === 'update' ? handlePasswordUpdate($event) : handleForgotSubmit($event)"
          @back="showAuth"
        />
      </div>
